...
# a) Probabilidad de necesitar exactamente 5 lanzamientos (éxito en el 5º intento)
k <- 5
prob_exact_5 <- dgeom(k - 1, p)   # dgeom(4, 0.5)
prob_exact_5

# b) Probabilidad de necesitar más de 3 lanzamientos (éxito después del 3er intento)
# P(necesitar > 3) = P(no éxito en los primeros 3 lanzamientos) = (1-p)^3
prob_more_3 <- (1 - p)^3
prob_more_3

# (Comprobación usando acumulada)
prob_more_3_check <- 1 - sum(dgeom(0:(3-1), p))  # 1 - P(éxito en ≤ 3)
prob_more_3_check

# c) Media y varianza del número de lanzamientos (contando el intento con éxito)
# Si X = #fracasos antes del éxito, E[X] = (1-p)/p, Var(X) = (1-p)/p^2
# Y = X + 1 => E[Y] = 1/p, Var(Y) = (1-p)/p^2
media_necesarios <- 1 / p
varianza_necesarios <- (1 - p) / (p^2)

list(
  prob_exact_5 = prob_exact_5,
  prob_more_3 = prob_more_3,
  prob_more_3_check = prob_more_3_check,
  media = media_necesarios,
  varianza = varianza_necesarios
)
```
...
# a) Probabilidad de anotar por primera vez en el tercer intento
k3 <- 3
prob_3rd <- dgeom(k3 - 1, p_basket)  # dgeom(2, 0.7)

# b) Esperanza del número de intentos (contando el intento exitoso)
esperanza_intentos <- 1 / p_basket

# c) Probabilidad de necesitar entre 2 y 5 intentos (inclusive)
# P(2 ≤ Y ≤ 5) = sum_{k=2}^{5} dgeom(k-1, p)
k_vals <- 2:5
prob_2_5 <- sum(dgeom(k_vals - 1, p_basket))

list(
  prob_3rd = prob_3rd,
  esperanza_intentos = esperanza_intentos,
  prob_2_5 = prob_2_5
)
```
...
# a) Número esperado de inspecciones hasta la primera defectuosa (contando la defectuosa)
esperanza_inspecciones <- 1 / p_def

# b) Probabilidad de que la primera defectuosa aparezca después de la pieza número 100
# P(Y > 100) = P(no defectuosa en las primeras 100 piezas) = (1-p)^100
prob_after_100 <- (1 - p_def)^100

# c) Demostración numérica de la propiedad de falta de memoria
# Tomemos s = 50, t = 10 (ejemplo)
s <- 50
t <- 10

# Usando variable Y = número de prueba del primer éxito (éxito en k-ésimo intento)
# P(Y > s + t | Y > s) = P(Y > t)
lhs <- (1 - p_def)^(s + t) / (1 - p_def)^s   # = (1-p)^{t}
rhs <- (1 - p_def)^t

list(
  esperanza_inspecciones = esperanza_inspecciones,
  prob_after_100 = prob_after_100,
  falta_memoria_check = list(
    s = s,
    t = t,
    lhs = lhs,
    rhs = rhs,
    iguales = abs(lhs - rhs) < .Machine$double.eps^0.5
  )
)
```
