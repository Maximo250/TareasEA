...
# a) P(X = 4) en 1 minuto
p_a <- dpois(4, lambda_min)

# b) P(0) en 30 segundos -> lambda_30s = lambda_min * 0.5
lambda_30s <- lambda_min * 0.5
p_b <- dpois(0, lambda_30s)

# c) Número esperado en una hora
esperado_hora <- lambda_min * 60

# d) P(8 <= X <= 12) en dos minutos -> lambda_2min = lambda_min * 2
lambda_2min <- lambda_min * 2
p_d <- ppois(12, lambda_2min) - ppois(7, lambda_2min)

# Mostrar resultados
cat("Ejercicio: Centro de atención\n")
cat("--------------------------------\n")
cat("a) P(X = 4 en 1 min) =", round(p_a, 6), "\n")
cat("b) P(0 en 30 seg) =", round(p_b, 6), "\n")
cat("c) Llamadas esperadas en 1 hora =", esperado_hora, "\n")
cat("d) P(8 <= X <= 12 en 2 min) =", round(p_d, 6), "\n")
```
...
```{r}
# Tasa: 1.5 defectos por 100 productos
rate_per_product <- 1.5 / 100

# Para 500 productos
n_prod <- 500
lambda_500 <- rate_per_product * n_prod  # = 7.5

# a) P(X = 8)
p_a <- dpois(8, lambda_500)

# b) P(X >= 10)
p_b <- 1 - ppois(9, lambda_500)

# c) ¿Cuántos productos para que P(al menos 1 defecto) = 0.95?
# P(at least 1) = 1 - exp(-lambda_total) = 0.95 -> lambda_total = -log(0.05)
lambda_req <- -log(0.05)
n_req <- ceiling(lambda_req / rate_per_product)

# d) Si la tasa baja a 1.2 por 100, nueva prob P(X >= 10) para 500 productos
rate_new <- 1.2 / 100
lambda_500_new <- rate_new * n_prod
p_d <- 1 - ppois(9, lambda_500_new)

# Mostrar resultados
cat("Ejercicio: Línea de producción\n")
cat("--------------------------------\n")
cat("a) P(X = 8 en 500 productos) = ", round(p_a, 6), "\n")
cat("b) P(X >= 10 en 500 productos) = ", round(p_b, 6), "\n")
cat("c) Productos necesarios para P(>=1 defecto)=0.95: ", n_req, "productos\n")
cat("   (lambda_total requerido =", round(lambda_req,4), ")\n")
cat("d) Nueva P(X >=10) con tasa 1.2/100 =", round(p_d, 6), "\n")
```
...
```{r}
lambda_hora <- 2.5  # pacientes por hora

# a) P(X = 3 en 1 hora)
p_a <- dpois(3, lambda_hora)

# b) P(0 en 2 horas) -> lambda_2h = 2 * lambda_hora
lambda_2h <- 2 * lambda_hora
p_b <- dpois(0, lambda_2h)

# c) Capacidad 8 pacientes por turno de 4 horas -> lambda_4h = 4 * lambda_hora
lambda_4h <- 4 * lambda_hora
p_c <- 1 - ppois(8, lambda_4h)  # P(X > 8)

# d) Capacidad mínima para que P(saturación)> capacidad < 0.10
# Buscamos el menor k tal que P(X > k) < 0.10
prob_limit <- 0.10
k <- 0
while ((1 - ppois(k, lambda_4h)) >= prob_limit) {
  k <- k + 1
}
cap_min <- k  # capacidad mínima para que P(X > cap_min) < 0.10

# Mostrar resultados
cat("Ejercicio: Hospital (urgencias)\n")
cat("--------------------------------\n")
cat("a) P(3 pacientes en 1 hora) =", round(p_a, 6), "\n")
cat("b) P(0 pacientes en 2 horas) =", round(p_b, 6), "\n")
cat("c) P(>8 pacientes en 4 horas) =", round(p_c, 6), "\n")
cat("d) Capacidad mínima para P(saturación)<10% en 4 horas:", cap_min, "\n")
```
...
medias_list <- lapply(sample_sizes, function(n) {
  replicate(num_samples, mean(rpois(n, lambda_tcl)))
})

# Resumen de medias y varianzas observadas vs teóricas
resumen <- data.frame(
  n = sample_sizes,
  Media_Obs = sapply(medias_list, mean),
  Media_Teor = lambda_tcl,
  Var_Obs = sapply(medias_list, var),
  Var_Teor = lambda_tcl / sample_sizes
)
print(round(resumen, 4))

# Gráficos: histogramas con curva normal teórica para cada n
plots <- lapply(seq_along(sample_sizes), function(i) {
  n <- sample_sizes[i]
  df <- data.frame(media = medias_list[[i]])
  ggplot(df, aes(x = media)) +
    geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "Purple", alpha = 0.6) +
    stat_function(fun = dnorm, args = list(mean = lambda_tcl, sd = sqrt(lambda_tcl / n)),
                  color = "#E74C3C", size = 1) +
    ggtitle(paste("n =", n)) + xlab("Media muestral") + ylab("Densidad") +
    theme_minimal()
})

# Mostrar los primeros 3 plots en consola (o usar patchwork/arrange si quieres todos)
library(gridExtra)
...
```{r}
# Verificación por simulación de la propiedad aditiva (corregido)
set.seed(1234)

# a) X1 ~ Poisson(3.5), X2 ~ Poisson(2.8)
lambda1 <- 3.5
lambda2 <- 2.8
n_sim <- 10000
...
# b) Poisson directa con lambda = 6.3
lambda_sum <- lambda1 + lambda2
poisson_directa <- rpois(n_sim, lambda_sum)

# Comparación estadística (creamos data.frame y redondeamos solo columnas numéricas)
stats <- data.frame(
  Metodo = c("Suma (X1+X2)", "Poisson directa"),
  Media = c(mean(suma_sim), mean(poisson_directa)),
  Varianza = c(var(suma_sim), var(poisson_directa)),
  stringsAsFactors = FALSE
)

# Redondear únicamente las columnas numéricas
numcols <- sapply(stats, is.numeric)
stats[numcols] <- lapply(stats[numcols], function(x) round(x, 4))

print(stats)

# Comparación de frecuencias (primeros valores)
max_k <- max(c(suma_sim, poisson_directa))
tab_sum <- prop.table(table(factor(suma_sim, levels = 0:max_k)))
tab_dir <- prop.table(table(factor(poisson_directa, levels = 0:max_k)))

comp <- data.frame(
  k = 0:max_k,
  P_suma = as.numeric(tab_sum),
  P_directa = as.numeric(tab_dir)
)
print(head(comp, 15))

# c) Extensión a la suma de 4 variables (ejemplo con lambdas c(1.5,2,2.5,3))
lambdas4 <- c(1.5, 2.0, 2.5, 3.0)
n_sim4 <- 10000
set.seed(4321)
vars_mat <- sapply(lambdas4, function(l) rpois(n_sim4, l))
suma4 <- rowSums(vars_mat)
poisson4_directa <- rpois(n_sim4, sum(lambdas4))

stats4 <- data.frame(
  Metodo = c("Suma 4 Poisson", "Poisson directa (λ total)"),
  Media = c(mean(suma4), mean(poisson4_directa)),
  Varianza = c(var(suma4), var(poisson4_directa)),
  stringsAsFactors = FALSE
)

numcols4 <- sapply(stats4, is.numeric)
stats4[numcols4] <- lapply(stats4[numcols4], function(x) round(x, 4))
print(stats4)
```
...
```{r}
lambda_min <- 15  # visitantes por minuto
capacidad1 <- 25
capacidad2 <- 50

# a) P(degradación) para capacidad 25
p_a <- 1 - ppois(capacidad1, lambda_min)

# b) P(degradación) para capacidad 50
p_b <- 1 - ppois(capacidad2, lambda_min)

# c) Capacidad para que P(degradación) < 0.01
prob_target <- 0.01
k <- 0
while ((1 - ppois(k, lambda_min)) >= prob_target) {
  k <- k + 1
}
cap_optima <- k

# d) Simulación durante 60 minutos (1 hora)
set.seed(2025)
minutos <- 60
sim_min <- rpois(minutos, lambda_min)
porcentaje_degradacion <- mean(sim_min > capacidad1) * 100

# Mostrar resultados
cat("Aplicación Web\n")
cat("----------------------------\n")
cat("a) P(degradación, cap=25) =", round(p_a, 6), "\n")
cat("b) P(degradación, cap=50) =", round(p_b, 6), "\n")
cat("c) Capacidad mínima para P(degradación)<1%:", cap_optima, "\n")
cat("d) Simulación 60 min: porcentaje de minutos con degradación (cap=25) =", 
    round(porcentaje_degradacion, 2), "%\n")
```
...
```{r}
# Inventarios: demanda Poisson(lambda=8) por día
lambda_day <- 8

# a) Si inventario inicial = 12, P(quedarse sin existencias) = P(D > 12)
s_init <- 12
p_a <- 1 - ppois(s_init, lambda_day)

# b) ¿Cuántas unidades para que P(agotarse) < 0.05?
target <- 0.05
s <- 0
while ((1 - ppois(s, lambda_day)) >= target) {
  s <- s + 1
}
s_min <- s

# c) Cálculo de costo esperado diario para diferentes niveles de inventario
# Costos:
c_hold <- 2   # por unidad no vendida
c_stockout <- 15  # por venta perdida
c_acq <- 10   # costo de adquisición por unidad (se aplica por cada unidad comprada al inicio del día)

max_k <- 30  # sumar probabilidades hasta este valor para aproximar cola
k_vals <- 0:max_k
p_k <- dpois(k_vals, lambda_day)

calc_expected_cost <- function(s) {
  # E[unsold] = sum_{k=0}^s (s - k) * P(D=k)
  unsold <- sum((s - k_vals[k_vals <= s]) * p_k[k_vals <= s])
  # E[shortage] = sum_{k=s+1}^\infty (k - s) * P(D=k)
  shortage <- sum((k_vals[k_vals > s] - s) * p_k[k_vals > s])
  # costo total esperado
  costo <- c_hold * unsold + c_stockout * shortage + c_acq * s
  return(list(costo = costo, unsold = unsold, shortage = shortage))
}

s_range <- 0:25
results <- t(sapply(s_range, function(si) {
  r <- calc_expected_cost(si)
  c(s = si, costo = r$costo, unsold = r$unsold, shortage = r$shortage)
}))

results_df <- as.data.frame(results)
optimal_row <- results_df[which.min(results_df$costo), ]

# Mostrar resultados
cat("Inventarios (lambda = ", lambda_day, ")\n", sep = "")
cat("-----------------------------------------\n")
cat("a) P(quedarse sin existencias con s=12) =", round(p_a, 6), "\n")
cat("b) Unidades para P(agotarse)<5%:", s_min, "\n\n")
cat("c) Ejemplo: cálculo de costo esperado para s=0:25 (mostrar los 5 mejores)\n")
print(head(results_df[order(results_df$costo), ], 5))
cat("\nNivel óptimo (min costo) -> s =", optimal_row$s, 
    ", costo esperado =", round(optimal_row$costo, 2),
    ", E[unsold] =", round(optimal_row$unsold,2),
    ", E[shortage] =", round(optimal_row$shortage,2), "\n")
```
