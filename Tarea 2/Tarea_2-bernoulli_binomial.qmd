...
# b) Media, varianza, desviación estándar
media_maq <- p
var_maq <- p * (1 - p)
sd_maq <- sqrt(var_maq)

# c) n = 10, probabilidad exactamente 2 defectuosas
n <- 10
prob_exact_2 <- dbinom(2, size = n, prob = p)

cat("Máquina (p = 0.02)\n")
cat("P(no defectuosa) =", prob_no_def, "\n")
cat("E[X] =", media_maq, " Var(X) =", var_maq, " SD =", sd_maq, "\n")
cat("P(X = 2) (n=10) =", prob_exact_2, "\n")
```
...
media_examen <- p_examen
var_examen <- p_examen * (1 - p_examen)
sd_examen <- sqrt(var_examen)

cat("Examen (p = 1/4)\n")
cat("P(acertar) =", p_examen, "\n")
cat("E[X] =", media_examen, " Var(X) =", var_examen, " SD =", sd_examen, "\n")
```
...
# a) P(exactamente 8 caras)
prob_8 <- dbinom(8, size = n_mon, prob = p_mon)

# b) P(al menos 12 caras) = 1 - P(X <= 11)
prob_ge_12 <- 1 - pbinom(11, size = n_mon, prob = p_mon)

# c) Media y desviación estándar
media_mon <- n_mon * p_mon
sd_mon <- sqrt(n_mon * p_mon * (1 - p_mon))

cat("Moneda (n=15, p=0.5)\n")
cat("P(X = 8) =", prob_8, "\n")
cat("P(X >= 12) =", prob_ge_12, "\n")
cat("E[X] =", media_mon, " SD =", sd_mon, "\n")
...
# a) P(exactamente 18 curas)
prob_18 <- dbinom(18, size = n_med, prob = p_med)

# b) P(entre 15 y 19 curas)
prob_15_19 <- sum(dbinom(15:19, size = n_med, prob = p_med))

# probabilidades individuales (opcional)
probs_15_19 <- dbinom(15:19, size = n_med, prob = p_med)
names(probs_15_19) <- paste0("k=", 15:19)

# c) Esperanza
esperado_med <- n_med * p_med

cat("Medicamento (n=20, p=0.85)\n")
cat("P(X = 18) =", prob_18, "\n")
cat("P(15 <= X <= 19) =", prob_15_19, "\n")
cat("Probabilidades individuales 15..19:\n")
print(probs_15_19)
cat("E[X] =", esperado_med, "\n")
```
...
X <- rbinom(nsim, n_t, p_t)
Y <- rbinom(nsim, m_t, p_t)
S <- X + Y

# Frecuencias empíricas y probabilidades teóricas para k = 0..(n+m)
k_all <- 0:(n_t + m_t)
emp_freq <- as.numeric(table(factor(S, levels = k_all))) / nsim
theo_prob <- dbinom(k_all, size = n_t + m_t, prob = p_t)

comp_df <- data.frame(
  k = k_all,
  Empirica = emp_freq,
  Teorica = theo_prob,
  Dif_abs = abs(emp_freq - theo_prob)
)

cat("Suma de Binomiales (simulación)\n")
cat("Parámetros: n =", n_t, " m =", m_t, " p =", p_t, " nsim =", nsim, "\n")
cat("Máxima diferencia absoluta entre empírica y teórica:", max(comp_df$Dif_abs), "\n\n")

print(comp_df)

# (Opcional) gráfico comparativo si tienes graphics habilitado
if (interactive()) {
  barplot(rbind(comp_df$Teorica, comp_df$Empirica),
          beside = TRUE, names.arg = comp_df$k,
          col = c("purple", "gold"),
          legend.text = c("Teórica", "Empírica"),
          main = "Bin(n+m,p) teórica vs Empírica",
          xlab = "k", ylab = "Probabilidad / Frecuencia")}
```
