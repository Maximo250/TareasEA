...
comparar_binneg_geom <- function(r, p, n_sim = 10000, seed = 123) {
  set.seed(seed)
  # Generar suma de r geométricas (rgeom devuelve número de fracasos antes del primer éxito)
  suma_geo <- replicate(n_sim, sum(rgeom(r, prob = p)))
  # Generar directamente de la binomial negativa
  binneg <- rnbinom(n_sim, size = r, prob = p)
  
  df <- data.frame(
    valor = c(suma_geo, binneg),
    tipo = factor(rep(c(paste0("Suma geométricas r=", r),
                        paste0("BinNeg directa r=", r)),
                      each = n_sim))
  )
  
  # Resumen estadístico (opcional, impreso)
  resumen <- aggregate(valor ~ tipo, data = df, FUN = function(x) c(mean = mean(x), var = var(x)))
  print("Resumen (Media y Varianza por método):")
  print(do.call(data.frame, resumen))
  
  # Gráfico comparativo (densidad y/o histograma)
  p_plot <- ggplot(df, aes(x = valor, fill = tipo)) +
    geom_histogram(aes(y = after_stat(density)), bins = 40, alpha = 0.6, position = "identity") +
    facet_wrap(~tipo, scales = "free_y") +
    labs(title = paste("Comparación: Suma de geométricas vs Binomial Negativa (r =", r, ", p =", p, ")"),
         x = "Número de fracasos",
         y = "Densidad") +
    theme_minimal() +
    theme(legend.position = "none")
...
# Ejemplo de uso: generar el gráfico (r = 4, p = 0.3, n_sim = 10000)
graf <- comparar_binneg_geom(r = 4, p = 0.3, n_sim = 10000, seed = 42)
print(graf)
```
...
```{r}
# Ejercicio: Baloncesto (p = 0.7, r = 8)
set.seed(2025)
p <- 0.7
r <- 8
n_sim <- 1000

# a) Modelado con la proposición: suma de r geométricas
# Cada geométrica cuenta los fracasos antes del 1er éxito; sumar r de ellas da los fracasos antes del r-ésimo éxito.

# b) Número esperado de tiros fallados (teórico)
esperado_teorico <- r * (1 - p) / p
desv_teorico <- sqrt(r * (1 - p) / (p^2))

cat("Baloncesto (p =", p, ", r =", r, ")\n")
cat("Esperado teórico (número de tiros fallados):", round(esperado_teorico, 4), "\n")
cat("Desviación estándar teórica:", round(desv_teorico, 4), "\n\n")

# c) Simulación de 1000 sesiones
sim_sesiones <- replicate(n_sim, sum(rgeom(r, prob = p)))
media_sim <- mean(sim_sesiones)
sd_sim <- sd(sim_sesiones)

cat("Simulación (n =", n_sim, ")\n")
cat("Media simulada:", round(media_sim, 4), "\n")
cat("Desviación estándar simulada:", round(sd_sim, 4), "\n")
cat("Diferencia entre teórico y sim. (media):", round(abs(media_sim - esperado_teorico), 4), "\n\n")

# Mostrar un pequeño resumen y un histograma
print(table(factor(sim_sesiones, levels = min(sim_sesiones):max(sim_sesiones))[1:10]))

library(ggplot2)
ggplot(data.frame(fracasos = sim_sesiones), aes(x = fracasos)) +
  geom_histogram(binwidth = 1, fill = "Purple", alpha = 0.7, boundary = 0) +
  geom_vline(xintercept = esperado_teorico, color = "Gold", linetype = "dashed", size = 1) +
  labs(title = paste("Simulación: Tiros fallados antes de conseguir", r, "éxitos (p =", p, ")"),
       subtitle = paste("Línea roja: esperado teórico =", round(esperado_teorico, 2)),
...
```{r}
# Ejercicio: Biotecnología (p_success = 0.3, r = 4)
set.seed(2025)
p_success <- 0.3
r <- 4

# a) Probabilidad de exactamente 12 pruebas fallidas
k <- 12
prob_exact_12 <- dnbinom(k, size = r, prob = p_success)
cat("a) P(X = 12) =", round(prob_exact_12, 8), "\n")

# b) Probabilidad de necesitar más de 20 pruebas fallidas
# P(X > 20) = 1 - P(X <= 20)
prob_more_20 <- 1 - pnbinom(20, size = r, prob = p_success)
cat("b) P(X > 20) =", round(prob_more_20, 8), "\n")

# c) Costo esperado en pruebas fallidas (costo por prueba fallida = 10,000)
costo_por_prueba <- 10000
esperado_fallos <- r * (1 - p_success) / p_success
costo_esperado <- esperado_fallos * costo_por_prueba
cat("c) Fallos esperados (teórico):", round(esperado_fallos, 4), "\n")
cat("   Costo esperado en pruebas fallidas: $", format(round(costo_esperado, 2), big.mark = ","), "\n\n")

# Verificación por simulación (opcional)
n_sim <- 100000
sim_valores <- rnbinom(n_sim, size = r, prob = p_success)
prob_exact_12_sim <- mean(sim_valores == 12)
prob_more_20_sim <- mean(sim_valores > 20)
costo_prom_sim <- mean(sim_valores) * costo_por_prueba

cat("Verificación por simulación (n =", format(n_sim, big.mark = ","), ")\n")
cat("  P(X=12) sim:", round(prob_exact_12_sim, 6), " (teórico:", round(prob_exact_12, 6), ")\n")
cat("  P(X>20) sim:", round(prob_more_20_sim, 6), " (teórico:", round(prob_more_20, 6), ")\n")
cat("  Costo promedio simulado: $", format(round(costo_prom_sim, 2), big.mark = ","), "\n")

# Pequeño histograma de la simulación
library(ggplot2)
ggplot(data.frame(X = sim_valores), aes(x = X)) +
  geom_histogram(binwidth = 1, fill = "Purple", alpha = 0.7, boundary = 0) +
  labs(title = paste("Simulación BinNeg(r =", r, ", p =", p_success, ")"),
