...
sigma_temp <- 5
n_med <- 1000
muestras_temp <- rnorm(n_med, mean = mu_temp, sd = sigma_temp)

# 2. ¿Qué porcentaje del tiempo la temperatura está fuera de especificación?
lim_inf <- 350 - 10
lim_sup <- 350 + 10

fuera_especificacion <- mean(muestras_temp < lim_inf | muestras_temp > lim_sup) * 100
dentro_especificacion <- mean(muestras_temp >= lim_inf & muestras_temp <= lim_sup) * 100

cat("Porcentaje fuera de especificación:", round(fuera_especificacion,2), "%\n")
cat("Porcentaje dentro de especificación:", round(dentro_especificacion,2), "%\n")

# 3. Si el proceso se descentra a mu = 355°C, ¿cómo cambia la calidad?
muestras_temp_355 <- rnorm(n_med, mean = 355, sd = sigma_temp)
fuera_355 <- mean(muestras_temp_355 < lim_inf | muestras_temp_355 > lim_sup) * 100
dentro_355 <- mean(muestras_temp_355 >= lim_inf & muestras_temp_355 <= lim_sup) * 100

cat("Descentrado a 355°C -> fuera de especificación:", round(fuera_355,2), "%\n")
cat("Descentrado a 355°C -> dentro de especificación:", round(dentro_355,2), "%\n")

# 4. ¿Qué sigma se necesita para tener 99.9% dentro de especificación?
# Para tener el 0.1% de fuera, queremos que P( |X - 350| > 10 ) = 0.001 <=> P( |Z| > 2 ) = 0.001
# P( 340 < X < 360 ) = 0.999 => P( |Z| < (10/sigma) ) = 0.999

z_lim <- qnorm(1 - 0.001/2) # Dos colas: 0.001/2 cada una
sigma_min <- 10 / z_lim

cat("Sigma máximo para 99.9% dentro de especificación:", round(sigma_min, 3), "°C\n")

# Visualización
df_viz <- data.frame(temp = muestras_temp)
ggplot(df_viz, aes(x = temp)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = 'skyblue', color = 'black', alpha = 0.7) +
  stat_function(fun = dnorm, args = list(mean = mu_temp, sd = sigma_temp), color = 'red', linewidth=1.3) +
  geom_vline(xintercept = c(lim_inf, lim_sup), color = "darkgreen", linetype = "dashed", linewidth = 1.2) +
  geom_vline(xintercept = mu_temp, color = "purple", linetype = "dotted", linewidth = 1) +
  annotate("rect", xmin = lim_inf, xmax = lim_sup, ymin = 0, ymax = Inf, alpha = 0.09, fill = "green") +
  labs(title = "Temperaturas del proceso (simulación muestra)",
       subtitle = "Zona verde: especificación aceptable",
...
# 1. Simula las ventas de 24 meses
ventas <- rnorm(24, mean = mu_ventas, sd = sd_ventas)

# 2. Probabilidad de ventas > 60,000
prob_60000 <- 1 - pnorm(60000, mu_ventas, sd_ventas)
cat("Probabilidad de ventas > 60,000:", round(prob_60000, 4), "\n")

# 3. ¿En cuántos meses se esperan ventas < 30,000?
prob_30000 <- pnorm(30000, mu_ventas, sd_ventas)
esperados_menor_30000 <- round(prob_30000 * 24, 2)
cat("Meses con ventas < 30000 (esperados):", esperados_menor_30000, "\n")

# 4. Rango intercuartílico de las ventas
iqr <- IQR(ventas)
cat("Rango intercuartílico (muestral):", round(iqr,2), "\n")

# 5. Porcentaje de meses con ventas > 45,000
porc_45000 <- mean(ventas > 45000) * 100
cat("Porcentaje de meses con ventas > 45000:", round(porc_45000,2), "%\n")
```
...
# 1. Simular 800 baterías
vidas <- rnorm(800, mean = mu_bat, sd = sd_bat)

# 2. Garantía que cubre al 95%
garantia_95 <- qnorm(0.05, mean = mu_bat, sd = sd_bat, lower.tail = FALSE)
cat("Garantía (al 95%):", round(garantia_95,2), "horas\n")

# 3. 10,000 baterías, garantía de 900 horas: ¿Cuántas fallan?
prob_fallan_900 <- pnorm(900, mean = mu_bat, sd = sd_bat)
fallan_900 <- round(10000 * prob_fallan_900)
cat("De 10,000 baterías con garantía de 900h, se espera que fallen:", fallan_900, "\n")

# 4. Comparar media y mediana simuladas con teóricas
cat("Media simulada:", round(mean(vidas),2), "; teórica:", mu_bat, "\n")
cat("Mediana simulada:", round(median(vidas),2), "; teórica:", mu_bat, "\n")

# 5. Percentil 10 de la distribución
perc10 <- qnorm(0.1, mean = mu_bat, sd = sd_bat)
cat("Percentil 10 de vida útil:", round(perc10,2), "horas\n")
```
...
# 1. Simula 1000 mediciones
errores <- rnorm(1000, mean = mu_err, sd = sigma_err)

# 2. Porcentaje entre -5 y +5
porc_5 <- mean(errores >= -5 & errores <= 5) * 100
cat("Porcentaje de errores entre -5 y +5:", round(porc_5,2), "%\n")

# 3. Probabilidad de error > 6 en valor absoluto
prob_mas6 <- 2 * (1 - pnorm(6, mu_err, sigma_err))
cat("Probabilidad de error > 6 (absoluto):", round(prob_mas6,4), "\n")

# 4. Porcentaje de error ≤ 3 ("preciso")
...
asimetria <- skewness(errores)
cat("Asimetría muestral:", round(asimetria,4), "\n")
```
...
# 1. Simula 200 estudiantes de cada grupo
a <- rnorm(200, mean = mu_a, sd = sd_a)
b <- rnorm(200, mean = mu_b, sd = sd_b)

# 2. ¿Qué porcentaje de Grupo A supera la media de Grupo B?
porc_grande_b <- mean(a > mu_b) * 100
cat("Porcentaje de Grupo A > media Grupo B:", round(porc_grande_b,2), "%\n")

# 3. Probabilidad de B < 70
prob_b70 <- pnorm(70, mean = mu_b, sd = sd_b)
cat("Probabilidad de Grupo B < 70:", round(prob_b70 * 100,2), "%\n")

# 4. Comparar distribuciones visualmente
library(ggplot2)
df <- data.frame(
  puntuacion = c(a, b),
  grupo = factor(rep(c("A", "B"), each = 200))
)
ggplot(df, aes(x = puntuacion, fill = grupo)) +
  geom_histogram(aes(y = after_stat(density)), position = "identity", alpha = 0.5, bins = 30) +
  stat_function(fun = function(x) dnorm(x, mu_a, sd_a), color = "blue", linewidth = 1.2, linetype = "dashed") +
  stat_function(fun = function(x) dnorm(x, mu_b, sd_b), color = "red", linewidth = 1.2, linetype = "dashed") +
  labs(title = "Comparación de puntuaciones (A: azul, B: rojo)", x = "Puntuación", y = "Densidad") +
  theme_minimal()

# 5. Percentil 90 de cada grupo
p90_a <- qnorm(0.9, mean = mu_a, sd = sd_a)
p90_b <- qnorm(0.9, mean = mu_b, sd = sd_b)
cat("Percentil 90 Grupo A:", round(p90_a,2), "\n")
cat("Percentil 90 Grupo B:", round(p90_b,2), "\n")
```
...
# 2. Calcula la media, mediana y desviación estándar muestral
media <- mean(muestra_altura)
mediana <- median(muestra_altura)
desvest <- sd(muestra_altura)
cat("Media:", round(media,2), " | Mediana:", round(mediana,2), " | SD:", round(desvest,2), "\n")

# 3. Porcentaje de estudiantes que mide más de 180 cm
porc_180 <- mean(muestra_altura > 180) * 100
cat("Porcentaje > 180 cm:", round(porc_180,2), "%\n")

# 4. Altura en el percentil 25
q25 <- quantile(muestra_altura, 0.25)
q25_teorico <- qnorm(0.25, mean = mu_altura, sd = sigma_altura)
cat("Percentil 25 (muestra):", round(q25,2), "\n")
cat("Percentil 25 (teórico):", round(q25_teorico,2), "\n")

# 5. Histograma con densidad teórica superpuesta
ggplot(data.frame(x = muestra_altura), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = 'skyblue', color = 'black', alpha = 0.7) +
  stat_function(fun = dnorm, args = list(mean = mu_altura, sd = sigma_altura), color = "red", linewidth = 1.2) +
  labs(title = "Histograma y densidad teórica: alturas de estudiantes",
