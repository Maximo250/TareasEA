...
medias_muestrales <- sapply(n_seq, function(n) mean(rbeta(n, a, b)))

# Gráfica de convergencia
df_media <- data.frame(n = n_seq, media_muestral = medias_muestrales)
ggplot(df_media, aes(x = n, y = media_muestral)) +
  geom_line(color="#1f77b4", size=1.1) +
  geom_hline(yintercept = media_teorica, color="red", linetype="dashed") +
  labs(title = "Convergencia de la media muestral a la media teórica",
       x = "Tamaño de muestra (n)",
       y = "Media muestral") +
  annotate("text", x = max(n_seq), y = media_teorica, label = sprintf("Media teórica = %.2f", media_teorica), hjust = 1.05, vjust = -0.7, color = "red", size = 3.5) +
  theme_minimal()
```
...
varianzas_muestrales <- sapply(n_seq, function(n) var(rbeta(n, a, b)))

# Gráfica de convergencia
df_var <- data.frame(n = n_seq, var_muestral = varianzas_muestrales)
ggplot(df_var, aes(x = n, y = var_muestral)) +
  geom_line(color="#2ca02c", size=1.1) +
  geom_hline(yintercept = var_teorica, color="red", linetype="dashed") +
  labs(title = "Convergencia de la varianza muestral a la varianza teórica",
       x = "Tamaño de muestra (n)",
       y = "Varianza muestral") +
  annotate("text", x = max(n_seq), y = var_teorica, label = sprintf("Var. teórica = %.3f", var_teorica), hjust = 1.1, vjust = 1.2, color = "red", size = 3.5) +
  theme_minimal()
```
...
```{r}
# Elegimos el mayor tamaño de muestra
n_max <- max(n_seq)
muestra_max <- rbeta(n_max, a, b)

# Valores para graficar la PDF teórica
x_vals <- seq(0, 1, length.out=400)
y_teorica <- dbeta(x_vals, a, b)

# Histograma de la muestra y PDF teórica superpuesta
df_hist <- data.frame(x = muestra_max)
ggplot(df_hist, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 40, fill="#a6cee3", color="gray40", alpha=0.8) +
  geom_line(data = data.frame(x = x_vals, y = y_teorica), aes(x = x, y = y), color = "red", size=1.1) +
  labs(
    title = sprintf("Histograma de la muestra (n = %d) y PDF teórica Beta(2,5)", n_max),
    x = "x", y = "Densidad"
  ) +
  theme_minimal()
```
