...
for (a in alpha_vals) {
  temp <- data.frame(
    ...
    parametro = factor(paste("α =", a), levels = paste("α =", alpha_vals))
  )
...
# Visualización del efecto del parámetro lambda (tasa)
x_seq <- seq(0, 8, length.out = 500)
alpha_fijo <- 2
...
for (b in lambda_vals) {
  temp <- data.frame(
    ...
    parametro = factor(paste("λ =", b), levels = paste("λ =", lambda_vals))
  )
...
lambda_effect <- ggplot(df_lambda, aes(x = x, y = y, color = parametro)) +
  geom_line(linewidth = 1.2) +
  labs(title = "Efecto del Parámetro λ en la Distribución Gamma",
...
alpha_tcl <- 2.5
lambda_tcl <- 1.8
tamaños <- c(5, 10, 20, 30, 50, 100)
N_sim <- 1000

# Almacenar resultados
medias_tcl <- do.call(rbind, lapply(tamaños, function(n) {
  data.frame(
    media = replicate(N_sim, mean(rgamma(n, shape = alpha_tcl, rate = lambda_tcl))),
    n = factor(n)
  )
}))
...
var_teorica <- alpha_tcl / lambda_tcl^2

# Datos para curva normal teórica por tamaño
normas <- do.call(rbind, lapply(tamaños, function(n) {
  xs <- seq(min(medias_tcl$media), max(medias_tcl$media), length.out = 300)
  data.frame(
    x = xs,
    y = dnorm(xs, mean = media_teorica, sd = sqrt(var_teorica/n)),
    n = factor(n)
  )
}))

# Varianza de las medias muestrales para cada tamaño
resumen <- medias_tcl %>% group_by(n) %>%
  summarise(
    media_muestral = mean(media),
    varianza_muestral = var(media),
    media_teorica = media_teorica,
    varianza_teorica_medias = var_teorica / as.numeric(as.character(n)),
    .groups = 'drop'
  )

knitr::kable(resumen, digits = 4, caption = "Medias y varianzas muestrales de las medias de gamma(2.5, 1.8)") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"))

# Graficar histogramas de las medias muestrales + normal teórica
ggplot(medias_tcl, aes(x = media)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "lightblue", color = "black", alpha = 0.7) +
  geom_line(data = normas, aes(x = x, y = y), color = "red", linewidth = 1.3) +
  facet_wrap(~ n, scales = "free", ncol = 2) +
  labs(
    title = "TCL: Histogramas de medias muestrales de gamma(2.5, 1.8) y normal teórica (rojo)",
    x = "Media muestral",
    y = "Densidad"
  ) +
  theme_minimal()
...
for (n in ns) {
  sumas <- replicate(N_sim, sum(rexp(n, rate = lambda_exp)))
  datos <- data.frame(suma = sumas)
  p <- ggplot(datos, aes(x = suma)) +
    geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "#6baed6", color = "black", alpha = 0.7) +
    stat_function(fun = function(x) dgamma(x, shape = n, rate = lambda_exp), color = "red", linewidth = 1.4) +
    labs(
      title = paste0("Suma de ", n, " exponenciales (λ = ", lambda_exp, "): histograma vs gamma(", n, ", ", lambda_exp, ")"),
      x = "Suma", y = "Densidad"
    ) +
    theme_minimal()
  print(p)
