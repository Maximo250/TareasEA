...
for (n in n_valores) {
  temp_df <- data.frame(
    x = x_vals,
    y = pchisq(x_vals, df = n),
    n = paste("n =", n)
  )
  df_cdf <- rbind(df_cdf, temp_df)
}

p_cdf_basico <- ggplot(df_cdf, aes(x = x, y = y, color = n)) +
  geom_line(size = 1.3) +
  scale_color_viridis_d(name = "Grados de\nLibertad") +
  xlim(0, 25) + ylim(0, 1) +
  labs(
...
grados_lib <- c(5, 20, 50)
muestras <- c(100, 1000, 5000, 10000)

for (n in grados_lib) {
  for (tam in muestras) {
    x <- rchisq(tam, df = n)
    medias_acum <- cumsum(x) / (1:tam)
    varianzas_acum <- sapply(1:tam, function(i) if (i > 1) var(x[1:i]) else NA)
    theoretical_mean <- n
    theoretical_var <- 2 * n

    # Media acumulada
    df_media <- data.frame(Tamano = 1:tam, Media = medias_acum)
    p1 <- ggplot(df_media, aes(x = Tamano, y = Media)) +
      geom_line(color = "blue") +
      geom_hline(yintercept = theoretical_mean, color = "red", linetype = "dashed") +
      labs(title = paste0("Convergencia de la Media (n = ", n, ", muestra = ", tam, ")"),
    ...
    # Varianza acumulada
    df_var <- data.frame(Tamano = 1:tam, Varianza = varianzas_acum)
    p2 <- ggplot(df_var, aes(x = Tamano, y = Varianza)) +
      geom_line(color = "darkgreen") +
      geom_hline(yintercept = theoretical_var, color = "red", linetype = "dashed") +
      labs(title = paste0("Convergencia de la Varianza (n = ", n, ", muestra = ", tam, ")"),
    ...
    if (tam == max(muestras)) {
      ...
      p3 <- ggplot(df_hist, aes(x = x)) +
        geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "skyblue", color = "black", alpha = 0.7) +
        stat_function(fun = function(val) dchisq(val, n), color = "red", linewidth = 1.3) +
        labs(title = paste0("Histograma vs Densidad Teórica, n = ", n),
             x = "x", y = "Densidad") +
        theme_minimal()
      print(p3)
    }
...
n_df <- 6             # puedes modificar el valor
tamaños <- c(5, 15, 30, 60, 120)
B <- 1000

medias <- do.call(rbind, lapply(tamaños, function(n) {
  data.frame(
    media = replicate(B, mean(rchisq(n, df = n_df))),
    n = factor(n)
  )
}))

# Para cada tamaño de muestra, superpone la normal teórica
mu <- n_df
sigma2 <- 2 * n_df

normas <- do.call(rbind, lapply(tamaños, function(n) {
  xx <- seq(min(medias$media), max(medias$media), length.out = 300)
  data.frame(
    x = xx,
    y = dnorm(xx, mean = mu, sd = sqrt(sigma2/n)),
    n = factor(n)
  )
}))

ggplot(medias, aes(x = media)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "orange", color = "black", alpha = 0.7) +
  geom_line(data = normas, aes(x = x, y = y), color = "red", linewidth = 1) +
  facet_wrap(~ n, scales = "free", ncol = 2) +
  labs(
    title = paste("TCL para Ji-cuadrada con df =", n_df),
    x = "Media muestral",
    y = "Densidad",
    caption = "Curva roja: Normal teórica"
  ) +
  theme_minimal()
```
...
set.seed(808)
n_sim <- 5000
for (grados in c(3,8,15)) {
  z <- matrix(rnorm(grados * n_sim), nrow = n_sim)
  suma_cuad <- rowSums(z^2)
  df_hist <- data.frame(x = suma_cuad)
  p <- ggplot(df_hist, aes(x = x)) +
    geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "plum", color = "black", alpha = 0.7) +
    stat_function(fun = function(val) dchisq(val, df = grados), color = "red", linewidth = 1.3) +
    labs(title = paste0("aditividad: suma normales^2 vs ji-cuadrada (n=", grados, ")"),
         x = "Suma de cuadrados", y = "Densidad") +
    theme_minimal()
  print(p)
}
```
...
for (n in n_vals) {
  ...
  p <- ggplot(df_hist, aes(x = x)) +
    geom_histogram(aes(y = after_stat(density)), bins = 60, fill = "#fef08a", color = "black", alpha = 0.6) +
    geom_line(data = normal_df, aes(x = x, y = y), color = "red", linewidth = 1.3) +
    labs(
      title = paste0("Aproximación Normal: n = ", n, " muestras = ", N),
      x = "x", y = "Densidad"
    ) +
    theme_minimal()
  print(p)
...
n1 <- 5; n2 <- 10
x1 <- rchisq(n_sim, df = n1)
x2 <- rchisq(n_sim, df = n2)
suma_chi <- x1 + x2

df_suma <- data.frame(x = suma_chi)
p <- ggplot(df_suma, aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "lightgreen", color = "black", alpha = 0.7) +
  stat_function(fun = function(val) dchisq(val, df = n1 + n2), color = "red", linewidth = 1.2) +
  labs(
    title = "Aditividad: Suma de dos ji-cuadrada independientes",
    subtitle = "n1 = 5, n2 = 10 | Teoría: chi-cuadrada de 15 grados de libertad",
    x = "x", y = "Densidad"
  ) +
  theme_minimal()
print(p)
