...
df_data <- data.frame(
  ...
  distribution = factor(
    rep(c("t(1)", "t(2)", "t(5)", "t(30)", "N(0,1)"), each = length(x)),
...
densidad_t <- ggplot(df_data, aes(x = x, y = density, color = distribution)) +
  ...
  scale_color_manual(
    values = c("t(1)" = "red", "t(2)" = "blue", "t(5)" = "green", "t(30)" = "orange", "N(0,1)" = "black")
  ) +
  labs(
...
resultados <- lapply(tamaños, function(tam) {
  muestra <- rt(tam, df = n_df)
  data.frame(
    tamaño = as.character(tam),  # <- asegúrate de que sea carácter para compatibilidad con "teórico"
...
teorico <- data.frame(
  tamaño = "teórico",
  media_muestral = 0,
  varianza_muestral = n_df / (n_df - 2)
)

resultados_df <- bind_rows(resultados, teorico) %>%
  mutate(across(where(is.numeric), round, 4))

kable(resultados_df,
      caption = "Medias y varianzas muestrales vs. teóricas para t(5)") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))

# Histograma muestra grande vs teorías
t1000 <- rt(1000, df = n_df)
ggplot(data.frame(x = t1000), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40, fill = '#b2df8a', color = "gray50", alpha = 0.7) +
  stat_function(fun = function(x) dt(x, df = n_df), color = "red", linewidth = 1.5) +
  stat_function(fun = dnorm, args = list(mean = 0, sd = 1), color = "blue", linetype = "dashed", linewidth = 1) +
  labs(
    title = "Histograma de muestra t(5): Curva roja = t(5), azul = normal estándar",
    x = "x", y = "Densidad"
  ) +
  theme_minimal()
```
...
ggplot(data.frame(T = T), aes(x = T)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40, fill = '#a6cee3', color = "gray30", alpha = 0.7) +
  stat_function(fun = function(x) dt(x, df = n), color = "red", linewidth = 1.5) +
  labs(
    title = "Relación t-normal-ji: Histograma de T vs t(10)",
    x = "T", y = "Densidad"
  ) +
  theme_minimal()
```
...
### Ejercicio 3: n muestras normales → t(n-1)
...
ggplot(data.frame(T = T_val), aes(x = T)) +
  geom_histogram(aes(y = after_stat(density)), bins = 35, fill = '#fdbf6f', color = "gray50", alpha = 0.7) +
  stat_function(fun = function(x) dt(x, df = n-1), color = "red", linewidth = 1.5) +
  labs(
    title = "Distribución de T (muestras normales) vs t(9)",
    x = "T", y = "Densidad"
  ) +
  theme_minimal()
```
