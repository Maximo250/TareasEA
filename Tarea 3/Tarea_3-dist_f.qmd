...
# Histograma de X^2
df_a <- data.frame(X2 = X2)
x_seq <- seq(0, max(X2, Y), length.out = 500)
pdf_F <- df(x_seq, 1, n)

ggplot(df_a, aes(x = X2)) +
  geom_histogram(aes(y = ..density..), bins = 60, fill = "#9ecae1", color = "gray50", alpha = 0.8) +
  geom_line(data = data.frame(x = x_seq, y = pdf_F), aes(x = x, y = y), color = "red", size = 1) +
  labs(
    title = bquote(X^2~" para X "~sim~t(.(n))~" y densidad teórica F(1,"~.(n)~")"),
    subtitle = "Histograma de X² y PDF de F(1, n) superpuesta",
    x = expression(X^2),
    y = "Densidad"
  ) +
  theme_minimal()
...
# Simula X ~ F(a, b) y calcula 1/X
X <- rf(N, a, b)
Xinv <- 1/X

# Simulación directa Y ~ F(b, a)
Y <- rf(N, b, a)

x_seq <- seq(0, quantile(Xinv, 0.99), length.out=500)
pdf_F_inv <- df(x_seq, b, a)

ggplot(data.frame(Xinv = Xinv), aes(x = Xinv)) +
  geom_histogram(aes(y = ..density..), bins = 60, fill = "#b2df8a", color = "gray40", alpha = 0.8) +
  geom_line(data = data.frame(x = x_seq, y = pdf_F_inv), aes(x = x, y = y), color = "red", size = 1) +
  labs(
    title = bquote("Histograma de"~1/X~"para X "~sim~F(.(a), .(b))~" + PDF teórica F("~.(b)~","~.(a)~")"),
    subtitle = "El histograma de 1/X coincide con F(b, a) teórica (roja)",
    x = expression(1/X),
    y = "Densidad"
  ) +
  theme_minimal()
...
# Simula X ~ chi^2(a), Y ~ chi^2(b), ambos independientes
X <- rchisq(N, a)
Y <- rchisq(N, b)

Z <- (X/a) / (Y/b)

# Simulación directa de W ~ F(a, b) para comparación
W <- rf(N, a, b)

x_seq <- seq(0, quantile(Z, 0.995), length.out=500)
pdf_F <- df(x_seq, a, b)

ggplot(data.frame(Z = Z), aes(x = Z)) +
  geom_histogram(aes(y = ..density..), bins = 60, fill = "#fdbf6f", color = "gray50", alpha = 0.75) +
  geom_line(data = data.frame(x = x_seq, y = pdf_F), aes(x = x, y = y), color = "red", size=1) +
  labs(
    title = bquote("Histograma de Z = (X/a)/(Y/b) con X "~sim~chi^2*"(a), Y "~sim~chi^2*"(b)"),
    subtitle = "La densidad estimada coincide con PDF teórica F(a, b) (rojo)",
    x = "Z = (X/a)/(Y/b)",
    y = "Densidad"
  ) +
  theme_minimal()
