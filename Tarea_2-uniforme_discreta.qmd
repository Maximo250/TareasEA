...
#| label: ex1-urna
#| echo: true
#| fig.cap: "Probabilidad de obtener un número par al extraer una bola de 1 a 8"

# Definición del espacio muestral
valores_urna <- 1:8

# Eventos pares
pares <- valores_urna[valores_urna %% 2 == 0]

# Probabilidades (uniforme discreta)
prob_por_valor <- rep(1/length(valores_urna), length(valores_urna))

# Probabilidad de obtener un número par
prob_par <- sum(prob_por_valor[valores_urna %in% pares])

# Mostrar resultados
tabla_urna <- data.frame(
  Valor = valores_urna,
  Probabilidad = prob_por_valor,
  Par = valores_urna %% 2 == 0
)

kable(tabla_urna, caption = "Espacio muestral y probabilidad (urna 1..8)") %>%
  kable_styling(bootstrap_options = "striped")

cat("\nNúmeros pares:", paste(pares, collapse = ", "), "\n")
...
# Media teórica de una uniforme discreta en {a,...,b}
media_teorica <- (a + b) / 2

# Varianza teórica: ((m^2 - 1) / 12) donde m = b-a+1
m <- n2
varianza_teorica <- ((m^2 - 1) / 12)

# También calculamos directamente desde los valores (verificación)
prob_igual <- rep(1/m, m)
media_calc <- sum(valores2 * prob_igual)
varianza_calc <- sum((valores2 - media_calc)^2 * prob_igual)

# Mostrar resultados
cat("Valores considerados:", a, "a", b, "\n")
cat("n (cantidad de valores):", n2, "\n\n")
cat("Media teórica:", media_teorica, "\n")
cat("Varianza teórica:", varianza_teorica, "\n\n")
cat("Verificación por cálculo discreto:\n")
cat("Media (calculada):", media_calc, "\n")
cat("Varianza (calculada):", varianza_calc, "\n")

# Tabla resumen
res2 <- data.frame(
  Estadistico = c("Media teórica", "Varianza teórica", "Media calculada", "Varianza calculada"),
  Valor = c(media_teorica, varianza_teorica, media_calc, varianza_calc)
)
kable(res2, caption = "Resumen: media y varianza (5..15)") %>% kable_styling(bootstrap_options = "striped")
```
...
```{r}
#| label: ex3-simulacion
#| echo: true
#| fig.cap: "Comparación entre frecuencias teóricas y observadas (500 simulaciones)"

# Parámetros
valores3 <- 0:4
n_sim <- 500
prob_teorica <- rep(1/length(valores3), length(valores3))

# Simulación
set.seed(2025)
muestra3 <- sample(valores3, size = n_sim, replace = TRUE, prob = prob_teorica)

# Frecuencias observadas y relativas
freq_obs <- table(muestra3)
freq_rel <- prop.table(freq_obs)

# Dataframe de comparación
comparacion3 <- data.frame(
  Valor = as.integer(names(freq_rel)),
  Prob_Teorica = prob_teorica,
  Frec_Observada = as.numeric(freq_rel),
  Frec_Observada_Abs = as.numeric(freq_obs)
)

kable(comparacion3, caption = paste("Comparación teórica vs observada (n =", n_sim, ")")) %>%
  kable_styling(bootstrap_options = "striped")

# Gráfico comparativo
comparacion_long <- comparacion3 %>%
  pivot_longer(cols = c(Prob_Teorica, Frec_Observada),
               names_to = "Tipo", values_to = "Probabilidad") %>%
  mutate(Tipo = recode(Tipo, Prob_Teorica = "Teórica", Frec_Observada = "Observada"))

library(ggplot2)
ggplot(comparacion_long, aes(x = factor(Valor), y = Probabilidad, fill = Tipo)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  scale_fill_manual(values = c("Teórica" = "gold", "Observada" = "salmon")) +
  labs(title = paste("Frecuencias teóricas vs observadas (n =", n_sim, ")"),
       x = "Valor",
